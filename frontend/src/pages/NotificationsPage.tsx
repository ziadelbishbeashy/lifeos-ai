...
import { TimePicker12hField } from "../components/TimePicker12h";
...
export function NotificationHistoryPage(){
  ...
  if(email.isPending && proactive.isPending)return <PageState title="Loading notifications" text="Checking what V-SPACE noticed…"/>;
  ...
  return <section className="workspace-page">
    <PageHeader eyebrow="Proactive intelligence" title="Notifications" description="V-SPACE notices meaningful changes and attention signals without waiting for you to ask." actions={<a href="/notifications/settings" className="secondary-button">Settings</a>}/>
    <ErrorBanner message={error}/>

    <article className="panel-card proactive-notification-panel">
      <div className="section-heading"><div><span className="panel-kicker">I15 · In-app</span><h2>V-SPACE noticed</h2><p>Generated from verified I14 events. These are suggestions only; V-SPACE does not execute workspace actions automatically.</p></div>{unread?<button className="secondary-button compact" type="button" onClick={()=>readAll.mutate()} disabled={readAll.isPending}>Mark all read</button>:null}</div>
      ...
      {notices.length?<div className="proactive-notification-list">{notices.map(item=><div className={`proactive-notification-item severity-${item.severity} ${item.status==="unread"?"is-unread":""}`} key={item.id}>
        ...
        <div className="proactive-notification-copy"><div className="proactive-notification-topline"><strong>{item.title}</strong><span>{item.status==="unread"?"New":item.status}</span></div><p>{item.message}</p><small>{item.created_at?new Date(item.created_at).toLocaleString():""} · Verified V-SPACE state</small></div>
      ...
      </div>)}</div>:<div className="dashboard-empty-state compact-empty-state"><div className="empty-state-icon">✓</div><h3>Nothing needs your attention</h3><p>V-SPACE will surface overdue work, blocked tasks, approaching deadlines, stale document intelligence, and completed analysis here.</p></div>}
    </article>
