...
import { Icon, PageSkeleton } from "../components/VSpaceUi";
import { DashboardAsk, TodayPlanPreview, RecentKnowledge } from "../components/DashboardWidgets";
...
export function DashboardPage() {
  ...
  if (query.isPending) return <PageSkeleton label="Opening your dashboard…"/>;
  if (query.isError || !query.data) return <div className="dashboard-empty-state"><div className="empty-state-icon">!</div><h3>Dashboard unavailable</h3><p>V-SPACE could not load your workspace data.</p><button className="dashboard-secondary-action compact" onClick={() => query.refetch()}>Try again</button></div>;
  ...
  async function createProposal(priority: TodayPriority, actionType: string) {
    ...
    try {
      ...
      setProposalError(error instanceof Error ? error.message : "V-SPACE could not prepare this action.");
    } finally {
  ...
  async function resolveProposal(mode: "confirm" | "dismiss") {
    ...
    try {
      ...
      setProposalError(error instanceof Error ? error.message : "V-SPACE could not complete this action.");
    } finally {
      setProposalBusy(false);
    }
  }
  ...
  return <>
    <section className="dashboard-hero intelligent-home-hero">
      <div className="dashboard-hero-copy"><span className="dashboard-eyebrow">Your space, your pace</span><h1>{greeting}, {firstName}<span className="vs-greeting-dot">.</span></h1><p>{home?.briefing.summary || "Here’s what needs your attention today."}</p></div>
      <div className="vs-dashboard-date"><Icon name="calendar"/><div><strong>{new Date(`${data.today}T12:00:00`).toLocaleDateString(undefined, { weekday: "long" })}</strong><span>{new Date(`${data.today}T12:00:00`).toLocaleDateString(undefined, { month: "long", day: "numeric", year: "numeric" })}</span></div></div>
    </section>
    <DashboardAsk/>
    ...
    <section className="dashboard-stat-grid" aria-label="Workspace statistics">
      ...
      <StatCard tone="red" title="Need attention" value={data.counts.overdue_tasks} detail={`${data.counts.blocked_tasks} blocked · review overdue work`} icon="M11 7h2v6h-2V7Zm0 8h2v2h-2v-2Zm1-13 10 18H2L12 2Zm0 4.1L5.4 18h13.2L12 6.1Z" />
    </section>

    <section className="dashboard-main-grid">
      <article className="dashboard-panel focus-panel today-intelligence-panel">
        <div className="dashboard-panel-heading"><div><span className="panel-kicker">Your next meaningful step</span><h2>Your Focus Today</h2></div>{home?.verified_from_state ? <span className="today-trust-badge"><i />Verified state</span> : <span className="live-status">Workspace data</span>}</div>
      ...
      <TodayPlanPreview date={data.today}/>
    </section>

    {home ? <details className="vs-home-details"><summary><Icon name="analytics"/>Workspace briefing <span>Insights, activity and study context</span></summary><HomeSignalsCard home={home} completionRate={data.completion_rate}/><section className="home-intelligence-grid" aria-label="Today intelligence details">
...
function TodayIntelligence({ today, onAction, busy }: { today: TodayIntelligenceData; onAction: (priority: TodayPriority, actionType: string) => Promise<void>; busy: boolean }) {
  ...
  return <div className="today-intelligence">
    <div className={`today-intelligence-summary attention-${today.attention_level}`}>
      ...
      <a href="/ask">Ask V-SPACE</a>
    </div>
    {priorities.length ? <div className="today-priority-list">{priorities.map((item, index) => <TodayPriorityCard item={item} rank={index + 1} key={`${item.project_id}-${item.category}-${item.title}`} onAction={onAction} busy={busy} />)}</div> : <div className="today-clear-state"><span>✓</span><div><strong>No ranked attention item right now</strong><p>V-SPACE checked your current project state and did not find a concrete blocker, overdue item, near deadline, or stale project signal that outranks your normal work.</p></div></div>}
    <div className="today-intelligence-footer"><span>{today.counts.reviewed_projects} project{today.counts.reviewed_projects === 1 ? "" : "s"} reviewed</span>{today.context_limited ? <span>Context capped by your V-SPACE limits</span> : <span>Read-only until you confirm an action</span>}</div>
  </div>;
}
