...
class LifeOSPDFViewer {
    constructor(modal) {
        ...
        this.ocrLayoutUrl = modal.dataset.dbPdfOcrLayoutUrl;
    ...
    async renderTextLayer(page, viewport) {
        ...
        try {
            ...
            const nativeSpans = Array.from(
                this.textLayer.querySelectorAll("span")
            ).filter((span) => String(span.textContent || "").trim());
            const nativeText = nativeSpans
                .map((span) => String(span.textContent || ""))
                .join(" ")
                .replace(/\s+/g, " ")
                .trim();
            const nativeUseful = (
                nativeText.length >= 24
                && tokenizeWords(nativeText).length >= 4
            );

            if (!nativeUseful) {
                await this.renderOCRTextLayer(viewport);
            } else {
                this.textLayer.dataset.ready = "1";
                this.textLayer.dataset.source = "native";
            }
    ...
    async getOCRPageLayout(pageNumber) {
        if (!this.ocrLayoutUrl) {
            return null;
        }

        if (this.ocrLayoutCache.has(pageNumber)) {
            return this.ocrLayoutCache.get(pageNumber);
        }

        try {
            const response = await fetch(
                joinUrl(this.ocrLayoutUrl, `page=${encodeURIComponent(pageNumber)}`),
                {
                    credentials: "include",
            ...
            if (payload?.layout_available && Array.isArray(payload?.words) && payload.words.length) {
                this.ocrLayoutCache.set(pageNumber, payload);
            }
            return payload;
        } catch (error) {
            console.warn("LifeOS could not load the OCR text layer.", error);
            return null;
        }
    }

    normalizeOCRBoxForRotation(word) {
    ...
    async renderOCRTextLayer(viewport) {
        const layout = await this.getOCRPageLayout(this.currentPage);
        const words = Array.isArray(layout?.words) ? layout.words : [];

        if (!words.length) {
            this.textLayer.dataset.ready = "0";
            this.textLayer.dataset.source = "none";
            return false;
        }
        ...
        words.forEach((word) => {
            ...
            const box = this.normalizeOCRBoxForRotation(word);
            const left = clamp(box.left, 0, 1) * viewport.width;
            const top = clamp(box.top, 0, 1) * viewport.height;
            const width = Math.max(1, clamp(box.width, 0, 1) * viewport.width);
            const height = Math.max(2, clamp(box.height, 0, 1) * viewport.height);

            const span = document.createElement("span");
            span.textContent = `${text} `;
            span.dataset.dbOcrWord = "1";
            span.style.left = `${left}px`;
            span.style.top = `${top}px`;
            span.style.width = `${width}px`;
            span.style.height = `${height}px`;
            span.style.fontSize = `${Math.max(4, height * 0.82)}px`;
        ...
        this.textLayer.dataset.ready = "1";
        this.textLayer.dataset.source = "ocr";
        this.setStatus(
            `Page ${this.currentPage} · selectable OCR text layer`
        );
        return true;
    }
    ...
    attachPendingSelectionToAsk() {
        ...
        window.dispatchEvent(new CustomEvent("lifeos-pdf-selection-context", {
            detail: {
                documentId: Number(this.modal.dataset.dbDocumentId || 0),
                text: this.attachedContext.text,
                page: this.attachedContext.page,
                section: this.attachedContext.section,
            },
        }));
...
function initPDFViewer(modal) {
    if (!modal || viewerInstances.has(modal)) {
        return viewerInstances.get(modal) || null;
    }

    const viewer = new LifeOSPDFViewer(modal);
    viewerInstances.set(modal, viewer);
    return viewer;
}

function initAvailablePDFViewers() {
    document.querySelectorAll("[data-db-pdf-modal]").forEach((modal) => {
        initPDFViewer(modal);
    });
}

function findViewerForDocument(documentId) {
    const wanted = String(documentId || "");
    const modals = Array.from(document.querySelectorAll("[data-db-pdf-modal]"));
    const modal = modals.find((item) => String(item.dataset.dbDocumentId || "") === wanted) || modals[0];
    return modal ? initPDFViewer(modal) : null;
}

function bootPDFViewerBridge() {
    initAvailablePDFViewers();

    const observer = new MutationObserver(() => initAvailablePDFViewers());
    observer.observe(document.documentElement, { childList: true, subtree: true });

    window.addEventListener("lifeos-open-pdf-workspace", (event) => {
        const detail = event?.detail || {};
        const viewer = findViewerForDocument(detail.documentId);
        if (viewer) {
            viewer.open(parsePage(detail.page || 1));
        }
    });
}

if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", bootPDFViewerBridge, { once: true });
} else {
    bootPDFViewerBridge();
}
